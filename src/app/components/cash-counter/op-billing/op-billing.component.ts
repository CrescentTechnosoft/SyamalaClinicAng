...
export class OPBillingComponent implements OnInit, OnDestroy {
  ...
  sendBill() {
    // const patientID= this.formGroup.get('ptId').value;
    // console.log(billNo);
    // this.service.sendBill(billNo).subscribe(res =>{
    //   console.log(res)
    // })
  }

  sendOldBill() {
    if (this.formGroup.get('billNo').value === '') {
      this.toastr.error('Select the Bill NO  to send Bill');

    } else {
      const oldpatientID = this.formGroup.get('billNo').value;
      this.service.sendBill(oldpatientID).toPromise()
        .then(res => {
          if (res['status']) {
